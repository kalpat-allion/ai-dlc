...
>
> **Phase 7's two artifacts carry 8 placeholders and coined only 2** — `{{DOCS_DIR}}`, `{{HANDOFF_DIR}}` — reusing 6: `{{API_SPEC_PATH}}`, `{{ADR_DIR}}`, `{{ARCHITECTURE_DIR}}`, `{{REQUIREMENTS_DIR}}`, `{{SECURITY_DOCS_DIR}}`, `{{COVERAGE_REPORT_PATH}}`. **The best reuse ratio of any phase — six of eight — and the pair count is still six.** Its contribution to this file is **seven names drafted and withdrawn against three different cases**, one of which is new: `{{DOCS_PLATFORM}}` was cut because the template can *look the fact up*, which makes the placeholder a stale answer with a name on it. See [the withdrawn names](#a-third-kind-of-case--two-names-drafted-then-withdrawn) and [the pre-fillable guards](#a-fourth-kind-of-case--names-that-would-disable-a-guard-by-being-filled). It also supplies the **second instance** of the file's blindest spot — see [the hardcoded counterpart](#one-hardcoded-counterpart--worse-than-a-synonym-pair).
>
> *(The counts above were also recounted rather than incremented. The header said 44 and the inventory said "All 45"; the templates carried **44**, so the inventory heading was one over before the Phase 5 build and was corrected to **58** across **38** templates then. Phase 7 takes it to **60** across **40**.)*
...
**Phase 7 adds a sub-case that is not about literals at all, and it needs its own name: a placeholder for a fact the template can *look up* is a stale answer with a name on it.** `{{DOCS_PLATFORM}}` (`Mintlify` / `GitBook` / `Docusaurus`) was drafted into `finalise-documentation` and withdrawn. It fails **none** of the tests above — the skill's rules (structure, spec-rendered API reference, the completeness loop, the stop before publish) are true under all three platforms, so this is not the portability-lie case. It was cut because **the skill's first step reads the docs build configuration anyway**, to learn the navigation and the voice already in use. A value filled at install goes stale the day the repo migrates platforms and **nothing anywhere reports the disagreement**; a config read cannot go stale, because it is re-read every run. The general form is worth stating because it is cheap to get wrong in the opposite direction too: *if the template already has to read the artefact that carries the fact, the placeholder adds a second answer that can drift from the first.* Prefer the read. `{{LINK_CHECK_COMMAND}}` was drafted alongside it and withdrawn for a **different and sharper** reason — see [the pre-fillable guards](#a-fourth-kind-of-case--names-that-would-disable-a-guard-by-being-filled).
...
**Phase 7 withdrew four more names against this case, and one of them is the sharpest instance in the file.**

- **`{{LINK_CHECK_COMMAND}}`** (for `finalise-documentation`) is the `{{PERF_TEST_ENV}}` failure arriving through a **narrow** fill rather than a wrong one. The documentation gate carries *"Links work (no 404s)"*. Filled with a checker scoped to internal links only — or run `--offline`, which is the default in half the tooling — the box passes over a set of dead external links, and **there is no surface anywhere on which the narrowing is visible**: the command ran, it exited zero, the item reports met. A pass made *reachable* that should not be, and a pass is the verdict that stops anyone else looking. The skill now runs whatever the repository itself configures and reports the item **unverified** when there is none, which is the failure everybody can see.
- **`{{TEAM_STACK}}`** (for `handoff-agent`) would have disabled the citation rule rather than a stop-and-ask. The agent's whole discipline is that technology choices are cited to the decision record that made them; a fill hands it the answer *without a citation*, and **an uncited claim that looks configured is worse than a `[VERIFY]`** — it reads as sourced to the one reader who cannot check it.
- **`{{SUPPORT_PERIOD_DAYS}}` and `{{SUPPORT_SLA}}`** are contract facts the recipient plans against. Instantiated once and reused for the next engagement, the agent states the **previous** engagement's terms, fluently, in the document nobody re-derives. The support agreement is the source; the agent cites it or marks `[VERIFY]`.
- **`{{RECIPIENT_TEAM}}`** is the same failure with a name on it — the wrong team addressed correctly throughout.
...
All 60, grouped by what they describe. **Where each is consumed** is authoritative — it is generated from the templates, not remembered.
...
| `{{ADR_DIR}}` | `docs/adrs/` — **the first name to span Phases 2 and 3**, and now four phases; the value `/adr` writes into is the value `/load-task-context` reads back out and the value `threat-model-verifier` reads to tell Landed from Partial. **Phase 7 makes the mismatch load-bearing in a new direction:** `handoff-agent` cites a decision record for every *"we chose X because Y"*, so a fill that differs from what `/adr` writes into ships a handoff document whose **every decision citation resolves to nothing** — delivered to a team with nobody left to ask. `finalise-documentation` links the same directory from the architecture section and is forbidden from copying a record into a page | `/adr`, `solution-architect`, `architecture-reviewer`, `/load-task-context`, `threat-modeler`, `threat-model-verifier`, `finalise-documentation`, `handoff-agent` |
| `{{ARCHITECTURE_DIR}}` | `docs/architecture/` — read-only for the Phase 7 consumer: `handoff-agent` writes the handoff document's architecture section **from** it and never edits it | `solution-architect`, `architecture-reviewer`, `threat-modeler`, `handoff-agent` |
| `{{SECURITY_DOCS_DIR}}` | `docs/security/` — the **directory** only. The filenames inside it stay literals in the templates (`threat-model.md`, `ai-threat-review.md`, `mitigation-verification-<release>.md`, the post-mortem seed) because the phase's gates and handoff read those exact names; a repo that renames them has edited the templates, not filled a value. **Phase 7 adds the first read-only consumer**: `handoff-agent` writes the security-posture section from these reports and marks `[VERIFY: ...]` rather than characterising a posture it did not read — so a fill pointing anywhere else makes that section silently thin rather than visibly wrong | `threat-modeler`, `threat-model-verifier`, `secret-leak-response`, `handoff-agent` |
...
| `{{COVERAGE_REPORT_PATH}}` | `coverage/coverage-summary.json` — the audit refuses to report any number it did not read from this file, and **`handoff-agent` carries the same refusal at higher stakes**: a coverage figure quoted in a handoff document is planned against for months by people who cannot re-derive it. A fill pointing at a stale or partial report makes both consumers confidently precise about the wrong tree | `e2e-and-coverage-engineer`, `handoff-agent` |
| `{{API_SPEC_PATH}}` | `docs/api/openapi.yaml` — the spec **file**. Deliberately **not** `{{API_BASE_PATH}}`, which is a URL prefix and a different fact. Both Phase 3 commands consume it for opposite reasons: one reads a section from it, the other refuses to hand-copy anything that renders from it; `load-test-engineer` takes endpoints, payloads and auth from it and invents none of the three; `finalise-documentation` **renders** the API reference from it and refuses both to hand-write what renders from it and to edit it to make a page easier to write | `/load-task-context`, `/write-module-readme`, `load-test-engineer`, `finalise-documentation` |
| `{{DOCS_DIR}}` | `docs/`, `apps/docs/content/`, `website/docs/` — the **documentation site's page root**, and `finalise-documentation`'s entire write scope. **Not a general "docs folder", and the distinction is the near-pair risk in this row:** `{{ADR_DIR}}`, `{{ARCHITECTURE_DIR}}` and `{{REQUIREMENTS_DIR}}` are read-only sources that may or may not sit *inside* it, and the skill links to them rather than rewriting them. A fill that collapses the two produces a documentation pass with write access to the decision records it is supposed to cite — and editing an ADR to make the docs read complete is the one failure that leaves no trace in the docs themselves | `finalise-documentation` |
| `{{HANDOFF_DIR}}` | `handoff/` — the agent's entire write scope, one file per invocation. The **filenames inside it stay literals** in the template (`handoff-document.md`, `post-handoff-retro.md`) because a receiving team is told those names in the support agreement; only the directory varies. Same shape as `{{SECURITY_DOCS_DIR}}` one phase over | `handoff-agent` |
...
**A second instance, and this one was caught *before* it was created rather than after.** `finalise-documentation` needs to locate the repository's alert runbooks — it links them from the operations section and **refuses to author or rewrite one**. `{{ALERT_RUNBOOK_DIR}}` was drafted for exactly that and withdrawn: [`06-cicd-devops/skill-prompts/observability-bringup/SKILL.md`](../aidlc-phases/06-cicd-devops/skill-prompts/observability-bringup/SKILL.md) writes `/docs/runbooks/alerts/` as a **literal**, in the step that derives the runbook filename from the alert name and again in its write-scope refusal. Minting a name here would have produced the same defect as the `api-contract-freeze` row above — one fact, one literal, one name — with the aggravating detail that the two templates are on **opposite sides of a routing boundary**: the observability skill *writes* those runbooks and the documentation skill is forbidden from touching them, so an inconsistent fill would have the documentation pass linking a directory the runbook author never writes to, and the gate line *"runbook per alert"* reporting against an empty path.

**The cheap moment is authoring, and this is the proof.** Withdrawing the name cost one decision; the `api-contract-freeze` literal is still unfixed two builds later because renaming inside a vendored template breaks every repo that already instantiated it. The Phase 7 skill locates runbooks by **reading**, which is the same resolution the `{{DOCS_PLATFORM}}` sub-case reaches from the other direction.

> **The general form is worth a sweep somebody has not run yet.** The reconciliation table catches *one fact, two names*. The new row above catches *one name, two actors*. **Neither catches *one fact, one literal and one name*** — and a literal is invisible to every check this file performs, because every check starts by finding `{{...}}`. Grepping the shipped templates for hardcoded paths that another template parameterises is a different routine from anything currently documented here. **Two instances are now recorded and only one was found by reading rather than by authoring**, which is the argument for running the sweep rather than trusting the next author to catch their own.
...
7. **Check whether the template already reads the artefact that carries this fact.** If it does, prefer the read and do not mint the name — a fill is a second answer that drifts from the first, silently, the day the repo changes. `{{DOCS_PLATFORM}}` was withdrawn on this rule; `{{ALERT_RUNBOOK_DIR}}` on rules 4 and 7 together.
