...
>
> **Status:** **33 of the 34 proposed artifacts are built** and ship as templates under `aidlc-phases/*/`. Together with Phase 3's seven specialist subagents — which predate this analysis and are therefore outside the 34 — that is **40 shipped artifacts** in the repo, the figure [`ROUTING.md`](./ROUTING.md) scopes to. **All seven phases are complete;** the single outstanding artifact is `sweep-requirements-gaps`, **held under a falsifiable release trigger rather than pending**. See [Build order](#build-order). *(This line previously read "analysis only. No skills, subagents, or commands have been authored" — false since the v2.4 Phase 6 build, and left standing through three subsequent ones. It then read "22 of the 34 … 29 shipped" through the Phase 4 build, which shipped four artifacts without touching either figure. Both are now **counted from the file tree** rather than incremented, which is the only method that cannot drift.)*
...
>
> **Revision — Phase 5 is now BUILT, and the two threat-model agents shipped as the pair this document required.** `threat-modeler`, `dependency-risk-analyst` and `threat-model-verifier` ship under `05-security/subagent-prompts/`; `codeql-query-author` and `secret-leak-response` under `skill-prompts/`. **No prompt was deleted, renamed or rewritten**, so counts hold at 96 prompts and 34 artifacts — with **31 of the 34 built** and **38 shipped artifacts in the repo**, both recounted from the tree rather than incremented, because the Phase 4 build shipped four artifacts without moving either figure. **The last outstanding coordinated edit in this document turned out not to be needed:** the `threat-modeler` ↔ `architecture-reviewer` mutual exclusion was carried as the one remaining bundled row, and the shipped Phase 2 reviewer already excludes threat modelling **by work type**, so `threat-modeler` shipped with the reciprocal written the same way and zero edits to another phase. **Three placeholder names were drafted and withdrawn for a reason this document had not seen before** — not because the fact is a literal elsewhere, but because filling it disables a stop-and-ask guard. See [Phase 5 build — consequences](#phase-5-build--consequences).
>
...
>
> **Revision — Phase 7 is now BUILT, and every phase has its set.** `finalise-documentation` ships under `07-delivery-handoff/skill-prompts/` and `handoff-agent` under `subagent-prompts/`. **No prompt was deleted, renamed or rewritten**, so counts hold at 96 prompts and 34 artifacts — with **33 of the 34 built** and **40 shipped artifacts in the repo**, both recounted from the tree (19 skills + 18 subagents + 3 commands). **`triage-inherited-errors` was not resurrected**, and this build is the last opportunity it had. Two gate lines were **added rather than tightened** — Gate 3 gained a redaction checkbox (6 items → 7) and Gate 2's completeness and link checks gained the qualifier that makes each falsifiable. **The two closing artifacts self-certify 15 of 19 and 1 of 7 gate items** — the lowest ratios in the framework, and the correct ones: delivery is where the checks are human signatures. **Seven placeholder names were withdrawn**, one of them in a failure direction this document had not recorded before. See [Phase 7 build — consequences](#phase-7-build--consequences).
...
**33 of the 34 are built; 1 remains.** Phase 6's eight shipped alongside the v2.4 Pulumi removal — see [Pulumi removal — consequences](#pulumi-removal--consequences) for why a vendor migration turned out to be the right moment to build a phase's helpers — Phase 1's three P0 skills shipped next, as the first artifacts built in this document's own recommended order, then Phase 2's seven as one set, Phase 3's remaining four, Phase 4's four, Phase 5's five, and Phase 7's two. Counted as **shipped artifacts in the repo** rather than as conversion outputs the number is **40**, because Phase 3's seven specialist subagents predate this analysis and are excluded from the 34 — 40 is the figure [`ROUTING.md`](./ROUTING.md) scopes to. The **1 remaining** is `sweep-requirements-gaps`, **held with a falsifiable release trigger — not pending.** Recounted by type: **19 of the 20 skills, 11 of the 11 subagents and 3 of the 3 commands.**

> **"Remaining" and "held" are now different populations, and the distinction has to survive this document.** Every previous banner could say *N remain* and mean *N not yet reached in the queue*. There is no queue left. What is outstanding is one artifact that a build **decided not to produce**, for a reason recorded with a `grep`-checkable trigger and a numeric drop condition. **An unbuilt artifact with no trigger is indistinguishable from an abandoned one**, and the only thing separating those two states here is that somebody wrote the condition down. When the count of remaining work reaches zero, the honest question stops being *what is next* and becomes *has anything we deferred expired.*

> **Both figures in this paragraph were stale for two builds, in the same way and for the same reason.** They read *"22 of the 34 … 29 shipped … 12 remaining"* through the entire Phase 4 build, which shipped four artifacts and updated the section headings, the build queue and the two cross-cutting docs — but not this paragraph. **A count maintained by hand in prose is a count that lags whichever edit somebody was actually focused on.** The fix applied here and in [PLACEHOLDERS.md](./PLACEHOLDERS.md) and the presentation deck is the same one: derive every figure from `aidlc-phases/*/{skill,subagent,command}-prompts/` at edit time and say in the text that it was derived, so the next reader can re-run the check in one command instead of trusting the arithmetic.
...
Priority split **of what remains**: **P0 = 0**, **P1 = 0** — all of them shipped. **P2 = 1**: `sweep-requirements-gaps`, held. That is 1, which reconciles with the Verdict.

> **The priority tiers have stopped doing any work, and it is worth saying why rather than letting them quietly empty.** Every tier was overridden by phase-set bundling — six times now — because artifacts that consume each other's outputs have to ship together whatever tier each carries. Phase 5 is the cleanest case: `codeql-query-author` was the phase's only P2 and shipped with four P1s, while `threat-model-verifier` **could not** have shipped before `threat-modeler` at any priority, because it audits that agent's output. Phase 7 closed the same way, a P1 skill with a P2 subagent. **Tiering ranks artifacts; dependency ranks builds; and where they disagree the second one has won every time — six for six.**
>
> **The tiers are now empty, and the one artifact that outlived them shows what they were missing.** `sweep-requirements-gaps` sat at P2 through six builds without P2 ever explaining what it was waiting *for*; a tier says what to build next and is silent the moment there is no next. What actually holds it is a pair of named triggers and a drop condition. **Priorities order a queue; only a trigger can expire a decision to wait** — and a tier applied to an artifact nobody intends to reach is a label, not a plan.
...
aidlc-phases/<phase>/
  ...
  subagent-prompts/<name>.md          # Phases 2, 3, 4, 5 and 6
  skill-prompts/<name>/SKILL.md       # Phases 1, 2, 3, 4, 5 and 6 — folders, not files
...
#### `secret-leak-response` — **SHIPPED** · Phase 5

- **Wraps:** Secrets Incident Response
- **Path:** [`aidlc-phases/05-security/skill-prompts/secret-leak-response/SKILL.md`](../aidlc-phases/05-security/skill-prompts/secret-leak-response/SKILL.md)
- **Gate:** P5 [Gate 5 (Secrets Hygiene)](../aidlc-phases/05-security/QUALITY-GATES.md#gate-5-secrets-hygiene) — sub-one-hour rotation MTTR, zero open incidents
- **Description:** see the shipped template; the frontmatter is the authoritative version.
...
- **What changed on build:**
  - **One trigger was cut as a routing defect, and it was the phrasing the pre-build description led with.** *"Do we need to rewrite git history"*, unqualified, is far more likely to be about squashing commits or dropping a large binary than about a leak — it would pull a fifteen-minute incident procedure into an ordinary git turn, and the repo's conflict-resolution work speaks the same language. Replaced with *"do we need to scrub this key out of git history"*; **every history-related trigger now names the credential.** Same shape as `run-sprint-planning`'s cut trigger: the collision was with ordinary English rather than with an incumbent artifact, so the fix was one qualifier rather than a narrowing.
  - **The classification step became worst-case-by-default, which the source prompt leaves open.** That prompt asks for first-exposure timestamp and audience "if known"; a leak triaged optimistically is triaged once. Unknown first exposure now means *since the credential was created*, and unknown audience means *public*.
  - **The audit window is the full live window, not the detection window.** The source prompt says "audit logs for unauthorised use" without bounding it, and the natural reading is detection-to-revocation. The exposure started at first commit. **A clean audit over a window shorter than the exposure is not a clean audit**, and the skill is required to say so when log retention does not reach back far enough — which on a months-old leak is the common case, not the edge case.
  - **`{{SECRETS_MANAGER}}` is reused rather than coined**, so a Phase 6 bring-up name now has an incident-time consumer. Recorded in [PLACEHOLDERS.md](./PLACEHOLDERS.md): filled with a store the runtime does not actually read, the rotation "succeeds" and the service keeps authenticating with the credential being killed.
  - **No coordinated edit was needed, and that was not luck.** `ci-identity-and-secrets-bringup` already excludes *"rotating a leaked credential during an incident"* by name, so the incident lane was reserved from the other side before this skill existed. Recorded in [`ROUTING.md`](./ROUTING.md) as a non-collision on purpose — a reserved lane reads identically to an oversight unless somebody writes it down.

#### `codeql-query-author` — **SHIPPED** · Phase 5
...
- **Path:** [`aidlc-phases/05-security/skill-prompts/codeql-query-author/SKILL.md`](../aidlc-phases/05-security/skill-prompts/codeql-query-author/SKILL.md)
- **Gate:** P5 [Gate 2 (SAST Quality Gate)](../aidlc-phases/05-security/QUALITY-GATES.md#gate-2-sast-quality-gate) — the custom-query checkbox, plus the escalation clause
- **Description:** see the shipped template; the frontmatter is the authoritative version.
...
- **What changed on build:**
  - **Two of the four proposed triggers were cut, and the second one is the more dangerous.** *"Lint for this pattern"* fires on somebody wanting an ESLint rule and returns a CodeQL query — a plausible, committable, entirely wrong artefact. *"Make sure nobody does this again"* is the closing sentence of a bug post-mortem, where the prevention action belongs to the bug procedure and its named owner; a query author winning that route substitutes a static-analysis rule for a process decision, and both look like progress. **Every surviving trigger names CodeQL, the query, or code scanning** — the same qualifier discipline that holds `author-test-plan` clear of "write the tests".
  - **It gained a step the prompt does not have: read the pack *before* writing, and prove the wiring *after*.** The source prompt asks for the `qlpack.yml` entry and the suite line as deliverables. **A query that is not in the suite the workflow runs is a file, not a control** — committed, reviewed, and never executed once, which is the manufactured-confidence failure the two fixtures exist to prevent, arriving one level up. Step 3 reads the existing pack and the workflow's `queries:` block; step 8 shows the workflow now runs one more query than it did.
  - **`{{CODEQL_LANGUAGES}}` is the only name coined here, and its failure direction is the good one.** A query written for a language the workflow does not build produces **no results and no error** — so the fill is checked against the workflow in step 2 rather than trusted.
  - **It is required to state what the two fixtures did *not* prove.** Two fixtures prove two directions on two samples; a skill that closes with "this query catches the pattern" has over-claimed in exactly the register the phase spends its time removing. Same property as the `PASS WITHHELD` family — see [the "I could not tell" token](#2-four-artifacts-needed-a-token-for-i-could-not-tell).
...
#### `finalise-documentation` — **SHIPPED** · Phase 7

- **Wraps:** Documentation Generation (per page), Doc Completeness Check
- **Path:** [`aidlc-phases/07-delivery-handoff/skill-prompts/finalise-documentation/SKILL.md`](../aidlc-phases/07-delivery-handoff/skill-prompts/finalise-documentation/SKILL.md)
- **Gate:** P7 [Gate 2](../aidlc-phases/07-delivery-handoff/QUALITY-GATES.md#gate-2-documentation-complete) (Documentation Complete) — **fifteen of its nineteen items**
- **Description:** see the shipped template; the frontmatter is the authoritative version.
...
- **What changed on build:**
  - **Fifteen of nineteen, and the four are named by owner.** Gate 2 carries thirteen required sections and six quality checks. The skill cannot certify **runbook-per-alert** (needs the live alert list — an alert configured in the observability backend but never committed as code is invisible to any repository read, and certifying it would be certifying `observability-bringup`'s gate line), **human review of auto-generated content** (it wrote the content), **screenshot and diagram currency** (it cannot see a rendered screen), or **site deployed and searchable** (it stops before publish by design). Enforced with a table on the `container-image-engineer` seven-of-nine precedent.
  - **`(use /write-module-readme)` was removed from the description and replaced by work type.** A cross-phase slug dangles in a repo that installed Phase 7 without Phase 3. The exclusion is unchanged in force, only in spelling — and the same treatment was applied to the *same-phase* pointer at `handoff-agent`, because this phase ships across two directories and installs per-directory.
  - **Two exclusions the spec did not have.** Alert runbooks — *"write the runbook"* is a reserved verb, and a documentation pass rewriting one produces a runbook that no longer matches the alert that fires it. And **editing the API specification itself**: the spec's *"hand-writing API reference pages"* covers only one direction, and a skill that edits the spec to make a page easier to write inverts the same truth flow.
  - **A disaster-recovery refusal was added.** Recovery objectives are stated by people, not derivable from a repository, and a generated DR plan reads authoritative while having never been tested. Reported as a gap with a named owner instead. **This is the `/infra-runbook` finding arriving from a second direction** — the third time an artifact has been stopped from generating an operational document whose only real content is a human commitment.
  - **The incumbent it collides with does not fight, which is worse than a collision.** `/write-module-readme` carries `disable-model-invocation: true`, so on *"document this module"* it would not compete — it would **silently lose**, handing a developer mid-story a whole-site documentation procedure. No reciprocal edit was needed: the command is `$ARGUMENTS`-scoped to one path and cannot take a site-wide request. **A disabled incumbent is not a mitigation; it is an absent defender.**
...
#### `threat-modeler` — **SHIPPED** · Phase 5 · `opus`

- **Wraps:** Threat Model STRIDE, AI Agent Threat Review
- **Path:** [`aidlc-phases/05-security/subagent-prompts/threat-modeler.md`](../aidlc-phases/05-security/subagent-prompts/threat-modeler.md)
- **Gate:** P5 [Gate 1 (Threat Model & Baseline)](../aidlc-phases/05-security/QUALITY-GATES.md#gate-1-threat-model--baseline) — the whole threat-model block; and [Gate 6 (AI Agent Security)](../aidlc-phases/05-security/QUALITY-GATES.md#gate-6-ai-agent-security) threat-surface block
- **Description:** see the shipped template; the frontmatter is the authoritative version.
...
- **What changed on build:**
  - **The prescribed coordinated edit turned out not to be needed, and this was the last bundled row in the document.** The `threat-modeler` ↔ `architecture-reviewer` mutual exclusion was carried as the one edit that had to bundle, because both sides were expected to name a slug. The **shipped** Phase 2 reviewer already excludes threat modelling by **work type** — *"the repo's security-review tooling"* — a genericisation made during the Phase 2 build to avoid naming an unbuilt artifact. So `threat-modeler` shipped with the reciprocal written the same way, and **the bundled-edit table is now empty.** Generalisable: **a genericisation adopted to avoid a dangling pointer also removes the coordinated edit that would otherwise have been owed later.** The cross-phase rule and the dangling-slug rule turn out to be the same rule seen from two ends.
  - **"Record `no finding` explicitly, per component, per STRIDE category" is not in the source prompt and is the difference between a model and a sample.** The prompt says walk every component through six categories; an agent that reports only what it found produces output in which **a skipped category and a clean one are indistinguishable**, and the reviewer cannot tell which they are reading. Same shape as `container-image-engineer` naming the two standards it cannot check.
  - **The AI pass is gated on the *product* shipping AI, not on the team using it.** PROCESS 6.1 makes the distinction and the source prompt does not carry it forward, so an agent reading helpfully will run the LLM Top 10 against a repo whose only AI is the developers' editor and return ten findings about a surface that does not exist.
  - **Two placeholders were drafted and withdrawn: `{{DATA_CLASSIFICATION}}` and `{{COMPLIANCE_SCOPE}}`.** They name real repository facts, and filling them **disables the stop-and-ask guard that severity is computed from** — a model rated against whatever was true at install, internally consistent and wrong about which findings block a launch. This is a fourth kind of placeholder failure, distinct from the three [PLACEHOLDERS.md](./PLACEHOLDERS.md#a-fourth-kind-of-case--names-that-would-disable-a-guard-by-being-filled) already tracked, and it now carries a naming rule of its own.
  - **It is forbidden to verify its own mitigations, in the body and not only the description.** A description constrains routing; once the agent is loaded, nothing in it constrains what the agent agrees to do next — which is exactly how *"and check they landed"* gets answered by the party that cannot answer it honestly.

#### `dependency-risk-analyst` — **SHIPPED** · Phase 5 · `opus`
...
- **Path:** [`aidlc-phases/05-security/subagent-prompts/dependency-risk-analyst.md`](../aidlc-phases/05-security/subagent-prompts/dependency-risk-analyst.md)
- **Gate:** P5 [Gate 3 (Dependency Hygiene)](../aidlc-phases/05-security/QUALITY-GATES.md#gate-3-dependency-hygiene) — the reachability-triage line and the major-bump impact line; feeds the 0-Critical and risk-acceptance bars
- **Description:** see the shipped template; the frontmatter is the authoritative version.
...
- **What changed on build:**
  - **The entry-point list is confirmed per run, and the placeholder that would have configured it was withdrawn.** `{{ENTRY_POINTS}}` looked like ordinary parameterisation — routes, CLI commands and scheduled jobs are a stable repo fact. **They are stable until the repo grows.** A list filled at install omits every route added since, and the CVEs reachable only through those routes come back `not reachable` carrying real import evidence behind them: **a Gate 3 pass made reachable that should not be**, arriving through staleness rather than a wrong value. The agent now enumerates candidates and requires human confirmation, and states the list it classified against at the top of every report so the reader can disagree with it.
  - **A distinction Gate 3 makes and the source prompt does not: shipped versus build dependencies.** The bar is *0 Critical **in production dependencies***. The prompt triages a flat CVE list. `{{DEPENDENCY_MANIFESTS}}` names the shipped tree, and the agent must say which side it put an ambiguous package on rather than assume — because in a monorepo the ambiguity is where the count comes from.
  - **The reachability caveat became a hard rule rather than a closing note.** The prompt lists *"treat reachability as a triage aid, NOT a definitive answer"* under Caveats, after the classification instructions. Read in order, a model has already decided before it reaches the caveat. In the shipped agent it is a hard rule with its consequence attached: **reachability ranks the work, it never clears a finding**, and the agent may not draft, propose or word the Tech Lead's exception.
  - **`{{TEAM_STACK}}` is reused, and it earns its place for a reason specific to this agent:** the stack is what decides *what counts as an unfollowable call path*. Reflection, dynamic dispatch and plugin loading are the reasons a verdict is `undetermined` rather than `not reachable`, and they are language facts.

#### `threat-model-verifier` — **SHIPPED** · Phase 5 · `opus`
...
- **Path:** [`aidlc-phases/05-security/subagent-prompts/threat-model-verifier.md`](../aidlc-phases/05-security/subagent-prompts/threat-model-verifier.md)
- **Gate:** P5 [Gate 7 (Compliance & Audit Readiness)](../aidlc-phases/05-security/QUALITY-GATES.md#gate-7-compliance--audit-readiness) and P5 [Phase Handoff](../aidlc-phases/05-security/QUALITY-GATES.md#phase-handoff) — both name this helper's source prompt as their producer and require `file:line` evidence per mitigation with **0 not landed**. The old *"or backlog"* loophole — the very thing that made the checkbox unfalsifiable — is gone; anything still not landed now carries a dated Tech Lead deferral. Traces back to Step 1.4, whose loop this closes at Step 1.5. *(Cited by line number here until the build; the two lines move on every gate edit, so the shipped template cites nothing and the gate cites the prompt by anchor.)*
- **Description:** see the shipped template; the frontmatter is the authoritative version.
- **What changed on build:**
  - **It gained a fourth status and a fourth verdict token, and that forced a gate edit.** The source prompt's guard is whole-run — *"if you cannot read the diff or the named module paths, STOP"* — which is safe and, on a real release, refuses constantly: twelve mitigations across eight modules, one of them in another repository, and the operator gets nothing. The shipped agent returns **`Unevaluated`** per row and **`PASS WITHHELD — N unevaluated`** as a verdict. **A new verdict token is a gate change, not a template detail** — the `accessibility-auditor` precedent — so Gate 7 and the Phase Handoff now read *"0 not landed **and 0 unevaluated**"*. Without that edit, `PASS WITHHELD` would have been read as a pass by anyone in a hurry, which is the unfalsifiable checkbox arriving from a third direction.
  - **Compound mitigations split into one row per control, and the reason is arithmetic.** *"Tag untrusted content, require confirmation before a state change, and lock the tool's parameters"* is three controls; two landed and one missing averages to Partial, and **Partial is a status a release ships on.** One row per control makes the missing third visible as Not landed.
  - **It refuses a same-session self-audit in the body, not only in the description.** The independence this pair exists for is a routing claim in the frontmatter and nothing more once the agent is loaded — the same reasoning that gave `architecture-reviewer` its self-review detection.
  - **Its drop condition was evaluated and did not fire.** The condition was *"build it only if the handoff checkbox is being ticked without evidence today"* — a fact about a team, not about the artifact, and therefore unanswerable at build time. Same resolution as `reproduce-and-diagnose-bug`: it ships carrying the condition as an **uninstall** instruction, phrased so that keeping it is the decision needing justification. **Second confirmation that a per-repo drop condition belongs in the install step, not in this queue.**
...
#### `handoff-agent` — **SHIPPED** · Phase 7 · `opus`

- **Wraps:** Handoff Document Generation, Post-Handoff Retrospective
- **Path:** [`aidlc-phases/07-delivery-handoff/subagent-prompts/handoff-agent.md`](../aidlc-phases/07-delivery-handoff/subagent-prompts/handoff-agent.md)
- **Gate:** P7 [Gate 3](../aidlc-phases/07-delivery-handoff/QUALITY-GATES.md#gate-3-handoff-package-ready) (Handoff Package Ready) — **one of its seven items**; feeds [Gate 5](../aidlc-phases/07-delivery-handoff/QUALITY-GATES.md#gate-5-support-period-closure) Retrospective
- **Description:** see the shipped template; the frontmatter is the authoritative version.
...
- **What changed on build:**
  - **One of seven, and that is the finding rather than a shortfall.** Gate 3 is a set of **human signatures**: review by Delivery Lead and Tech Lead, redaction resolution, *honest and complete*, *specific and actionable*, recipient iteration, and whether the inputs it was handed were *all* of them. The agent certifies exactly one — *all sections complete*, evidenced by per-section marker counts. **The agent's contribution to this gate is the evidence underneath a signature, never the signature.** A template built to report a pass would have fabricated six rows.
  - **The gate gained a seventh item, and this is the substantive gap the build found.** Gate 3 said nothing about redaction while the phase's own risk table names the customer-facing/internal leak as a top risk — so *marking* a breach and *silently deleting* it had **identical gate consequences**, which is precisely what makes deletion the tempting option. Gate 3 now requires every `[REDACT-BEFORE-SHARE]` marker resolved by the Tech Lead before the draft leaves the delivery team, and the agent states that **a zero count from it is only zero *found*.** Item count 6 → 7, with every "one of six" reference reconciled in the same change.
  - **It prints an artefact manifest before writing, and that is what makes Gate 3 line 1 checkable at all.** *"Generated with all Phase 1–6 inputs"* had no verification surface: the agent knows what it read and can never know what it was never handed. The manifest records **read** (with path) / **named but unreadable** / **not supplied** per expected source, and the gate now reads against it rather than against an assertion. **Reconstructed after the fact it is worth nothing**, which is why it is step 1 and not a closing summary.
  - **Marker counts are reported per section at the top.** Burying `[VERIFY]` at the point of use makes a draft with fourteen open questions look finished — the same reason `/load-task-context` reports `unchecked` per row rather than in aggregate.
  - **Never both deliverables in one run.** They are separated by the whole support period and read different evidence; merging them produces a retrospective written before the events it reports. Not in the spec, and the "generate the handoff doc and the retro in one go" request is the one the README makes a required boundary test.
  - **Never read or transcribe a credential value.** The phase's credential-inventory flow is explicit that Claude receives item and field names only; nothing in the spec carried that rule into the agent that reads the same vault-adjacent material.
  - **A new adjacency, not previously recorded anywhere: post-mortem vs retrospective.** Same noun family, different evidence and a different clock — a retrospective reports a support period that has ended, a post-mortem an incident that just happened. Closed by explicit exclusion.
...
>
> **Four instances is enough to invert the default.** The "Ships with" column should read *standalone* for any edit that is a pure refusal or a work-type restatement, and bundle only where a slug is genuinely named. **After Phase 3 the bundled set was down to one row** — the `threat-modeler` ↔ `architecture-reviewer` mutual exclusion, bundled because both sides were expected to name a slug and neither sentence survives its removal. The two rows that named `open-pull-request` landed with it.
>
> **After Phase 5 the bundled set is empty, and the last row was cancelled rather than paid.** The premise — *both sides name a slug* — was already false: the **shipped** Phase 2 reviewer restates the exclusion **by work type**, a genericisation made during that build precisely because `threat-modeler` did not exist to be named. So the Phase 5 build wrote the reciprocal in the same register and touched no Phase 2 file. **A genericisation adopted to avoid a dangling pointer also cancels the coordinated edit it would otherwise have owed** — which means the two rules this document keeps stating separately are one rule, and following either eagerly settles the other. See [Phase 5 build — consequences](#phase-5-build--consequences) §1.
...
| ~~`threat-modeler` ↔ `architecture-reviewer`~~ **CANCELLED, not shipped** | Mutual exclusion between the two subagents. Both `opus`, both read-only, both emit severity-ranked design-time findings, and "what are the security risks in this design" vs "what breaks in this design before we code it" are paraphrases — all still true. **But the edit was never owed:** the reviewer shipped in Phase 2 excluding threat modelling *by work type*, so `threat-modeler` shipped with the reciprocal in the same register and **no Phase 2 file was touched.** | ~~both (P1)~~ **neither** |
...
## Phase 5 build — consequences

Five artifacts, no prompt deleted, renamed or rewritten. Five findings worth keeping.

### 1. The last coordinated edit in this document was already paid for

The `threat-modeler` ↔ `architecture-reviewer` mutual exclusion was carried as the **one remaining bundled row** in [Extend, don't create](#extend-dont-create) — bundled specifically because both sides were expected to name a slug and neither sentence survives its removal. It did not need building. The **shipped** Phase 2 reviewer excludes threat modelling by **work type** (*"the repo's security-review tooling"*), a genericisation made during that build to avoid naming an artifact that did not yet exist, and `threat-modeler` shipped with the reciprocal written the same way.

**Generalise it: a genericisation adopted to avoid a dangling pointer also cancels the coordinated edit that would otherwise be owed later.** The cross-phase slug rule and the dangling-pointer rule turn out to be one rule seen from two ends, and following either one eagerly pays the other one off. The bundled-edit table is now empty.

### 2. A verdict token forced a gate edit, exactly as the accessibility precedent predicted

`threat-model-verifier` needed a per-row **`Unevaluated`** and a **`PASS WITHHELD`** verdict, because the source prompt's whole-run STOP refuses on any release where one affected module sits outside the diff — which is most of them. The moment that token existed, Gate 7's *"0 not landed"* and the Phase Handoff's identical line became **satisfiable by a `PASS WITHHELD`** to anyone reading quickly. Both now read *"0 not landed and 0 unevaluated"*.

This is the second time the rule has fired and the first time it was applied without being rediscovered: **a new verdict token is a gate change, not a template detail.** The repo now has three of them — `PASS WITHHELD` in two phases, seven-of-nine in containers, `undetermined` here — and in every case the gate had to be told the token exists, or the honest answer would have been read as the favourable one.

### 3. A placeholder failure mode this file had not seen: the pre-fillable guard

Three names were drafted and withdrawn — `{{DATA_CLASSIFICATION}}`, `{{COMPLIANCE_SCOPE}}`, `{{ENTRY_POINTS}}` — for a reason unlike the Phase 4 withdrawals. Those two named facts written as literals elsewhere. **These three name facts the template deliberately stops to ask for.** A placeholder is a way to pre-answer that question once and never be asked again, and the placeholder wins on the first run and every run after.

`{{ENTRY_POINTS}}` is the sharpest: a route list filled at install omits every route added since, so the CVEs behind them return `not reachable` with real import evidence attached, and **Gate 3 passes against a tree nobody re-counted.** Same failure direction as `{{PERF_TEST_ENV}}` — a pass made reachable that should not be — arriving through **staleness** rather than a wrong value, which is a slower and quieter path to the same place. [PLACEHOLDERS.md](./PLACEHOLDERS.md#a-fourth-kind-of-case--names-that-would-disable-a-guard-by-being-filled) carries it as naming rule 6: *check whether the template stops to ask for this fact.*

### 4. Two lanes were reserved from the other side before either artifact existed

`ci-identity-and-secrets-bringup` already excluded *"rotating a leaked credential during an incident"*; the Phase 2 design agents already excluded threat modelling. **Two of Phase 5's five needed no coordinated edit at all, and neither was luck** — both exclusions were written by earlier builds against artifacts that did not exist yet. Recorded in [`ROUTING.md`](./ROUTING.md) as non-collisions **on purpose**, because a reserved lane and an oversight are indistinguishable in a file that only records problems, and the first person to tidy the exclusion out re-opens it.

### 5. Two skill triggers were cut, and the more dangerous one collided with a sibling's terminal step

`codeql-query-author`'s proposed *"make sure nobody does this again"* is the closing sentence of a bug post-mortem — where `reproduce-and-diagnose-bug`'s step 7 seeds exactly one prevention action with one named owner. A query author winning that route **substitutes a static-analysis rule for a process decision, and both outputs look like progress.** `secret-leak-response`'s proposed *"do we need to rewrite git history"* is, unqualified, about squashing commits far more often than about a leak.

Neither collision is with an incumbent artifact's *name*; both are with **ordinary English that another artifact's procedure ends in**. The narrowing rule that has now held four times — the test-plan skill, the sprint-planning skill, and both of these — is one qualifier per trigger: name the artifact the procedure produces (test plan, cycle, CodeQL query, the credential), never the intent behind it.

---

## Phase 7 build — consequences

Two artifacts, no prompt deleted, renamed or rewritten. Six findings worth keeping, and the phase is the last one, so two of them are about what an empty queue does to this document.

### 1. The lowest self-certification ratios in the framework, and they are the honest ones

`finalise-documentation` certifies **15 of 19**; `handoff-agent` certifies **1 of 7**. Every prior build reported a ratio close to whole — seven of nine was the previous low — and the reflex on seeing one-of-seven is that the artifact is too weak to be worth building.

That reads the ratio backwards. **Delivery is the phase where the gate items are human signatures**, because it is the phase where the people who could check the work are leaving. *Reviewed by the Delivery Lead*, *known limitations honest and complete*, *draft shared with the recipient and iterated on* — none of these is a claim any agent could make truthfully, and an agent that reported them would be manufacturing exactly the assurance the recipient has no way to test. **The correct contribution to a gate made of signatures is the evidence underneath them.** The ratio is low because the gate is mostly human, which is the right shape for this gate — and a build that had "improved" the number would have done so by fabricating rows.

Generalise it: **a self-certification ratio is a property of the gate, not a score for the artifact.** Comparing them across phases ranks the gates by how automatable they are, and rewards building helpers where the checks are cheap.

### 2. A gate item was *added*, and the omission it closed made silent deletion the cheaper option

Every prior gate edit in this document **tightened** an existing line — `PASS WITHHELD`, *0 not landed and 0 unevaluated*, the `unchecked` row. This one added a checkbox. Gate 3 said nothing about `[REDACT-BEFORE-SHARE]` while the phase's own risk table named the customer-facing/internal leak as a top risk.

The consequence is worth stating precisely, because it is not "a check was missing". **With no gate line, marking a breach and silently deleting it had identical gate outcomes — and marking is the one that leaves work for a human.** The mark-never-drop rule was therefore load-bearing in the template and *unrewarded* at the gate, which is the exact configuration under which a rule quietly stops being followed. Gate 3 went 6 items → 7.

**A rule that costs the artifact something and buys it nothing at the gate is a rule with a decay clock on it.** That test is worth running against the other mark-rather-than-resolve rules in the repo.

### 3. A third failure direction for placeholders: the narrow fill

`{{LINK_CHECK_COMMAND}}` was drafted and withdrawn. Filled with a checker scoped to internal links — or run `--offline` — *"Links work (no 404s)"* passes over a set of dead external ones, **and nothing anywhere shows it.** The value is not wrong and it is not stale; it is **narrower than the claim it satisfies**, and the gap is invisible from both sides. Same destination as `{{PERF_TEST_ENV}}` (a pass made reachable that should not be) by a third route: wrong value, stale value, and now **narrow scope**.

`{{DOCS_PLATFORM}}` names a fourth: **a placeholder for a fact the template can look up is a stale answer with a name on it.** The skill reads the docs build configuration and learns the platform on every run; a fill learns it once and is wrong from the day the repo migrates. The naming rule this adds is *before minting a name, check whether the template could read the fact instead* — and prefer reading, because a config read cannot go stale.

Seven names were withdrawn in total, against **8 used and only 2 new** — the best reuse ratio of any phase, which is what a reconciliation file is supposed to produce once it exists.

### 4. `{{ALERT_RUNBOOK_DIR}}` is the second instance of the blindest spot, and the first one caught before minting

[PLACEHOLDERS.md](./PLACEHOLDERS.md) names *one fact, one literal, one name* as the case no check catches, because every check starts by grepping `{{...}}`. `observability-bringup` writes the alert-runbook path as a **literal**; coining a name for the same path here would have created the case a second time. It was caught by asking, before minting, *who else already writes this path* — which is a question a per-phase README structurally cannot answer and the reconciliation file can.

### 5. A disabled incumbent is not a mitigation

`/write-module-readme` carries `disable-model-invocation: true`. Against *"document this module"*, `finalise-documentation` does not **beat** it — the command is not in the race, and a developer mid-story receives a whole-site documentation procedure. **That is worse than a collision**: a collision is two artifacts contending and one winning, which somebody eventually notices; this is an uncontested route into the wrong artifact. No reciprocal edit was needed because the command is `$ARGUMENTS`-scoped and cannot take a site-wide request — but the boundary rests entirely on the *new* artifact's exclusion, and that asymmetry should be assumed wherever a user-invocable-only artifact shares a verb with an auto-triggering one.

### 6. What an empty queue exposes: a tier is not a reason to wait

`sweep-requirements-gaps` sat at P2 across six builds. P2 never said what it was waiting *for* — a tier orders a queue and says nothing once the queue is empty. What actually holds the artifact is a pair of named triggers and a numeric drop condition, written during the Phase 1 build precisely because the original hold reason (*"build it after the Gate 1/2 skills have run on a real PRD"*) could not be checked by anyone reading the repo.

**Priorities order a queue; only a trigger can expire a decision to wait.** With the queue empty, this is the difference between one held artifact and one abandoned one, and the only thing separating those two states in this repo is that somebody wrote the condition down in a form `grep` can settle.

### 7. Defects found in the Phase 7 source docs, and one that will mislead an operator

Five, recorded rather than fixed because they sit outside the build's scope — but the second is the one to action:

1. **`PROCESS.md:180` ships a starter `handoff-agent` frontmatter block describing the four-deliverable version**, the exact shape this analysis narrowed away from. **Anyone following Step 0.7 literally will commit the wide agent** — the narrowing is defeated by the phase's own setup instructions. Step 0.7 should point at the shipped template instead. Highest-value follow-up in the phase.
2. `PROCESS.md:175` and `:202` still read *"Do not create the file in this task"* and *"(If adopted)"*. The template now exists; the optional framing is stale and should become an install instruction.
3. **Gate 3's preamble asserts a check its own list does not carry** — it says the document is *"complete, fact-checked, and shared"*, but *fact-checked* appears in no checkbox. The nearest line is the review signature, which PROCESS 3.4 defines as including a cross-artefact fact-check. Needs a wording decision, not a mechanical edit.
4. **Gate 2 and PROCESS Step 7 disagree on where FAQ, glossary and troubleshooting live** — Gate 2 lists them as documentation-site sections; Step 7 seeds them into the knowledge base as a separate artefact with separate owners. Both are defensible and can coexist, but nothing says so, and **a team reading only Gate 2 will tick those two boxes against KB pages the documentation skill never touched.** The skill scopes itself to the site and excludes the KB by phrase; the gate needs the same qualifier.
5. Broken anchor at `PROCESS.md:531` — `#knowledge-base-seeding-faq-glossary-troubleshooting`, where GFM renders the `/` separators as **double** hyphens. The two new READMEs use the correct form. All seven other `PROMPTS.md` anchors in that file resolve.
...
No-coordinated-edit artifacts first: ~~**`load-test-engineer`** (zero routing collisions — the cheapest P1 to land)~~ **SHIPPED with the Phase 4 set**; ~~**`dependency-risk-analyst`**, `threat-modeler`, `secret-leak-response`, and **`threat-model-verifier` after `threat-modeler`, never before it**~~ — **all four SHIPPED with the Phase 5 set**, and the pair rule held: the verifier audits the modeller's output, so the two shipped in one PR with mutually-excluding descriptions. Then the remaining multi-step skills: ~~`author-test-plan`, `reproduce-and-diagnose-bug`~~ **both SHIPPED with the Phase 4 set**, and ~~`finalise-documentation`~~ **SHIPPED with the Phase 7 set**.

> **This queue is now empty.** Of the 41 helpers, **40 are built** — counted from `aidlc-phases/*/{skill,subagent,command}-prompts/` — and the one outstanding is `sweep-requirements-gaps`, which is **held rather than queued**. The queue's tier ordering was overridden by phase-set bundling **six** separate times (items 6, 7, 8, 9, Phase 5 and Phase 7), which is enough repetitions to state the rule plainly rather than as a series of exceptions: **artifacts that consume each other's outputs ship together, whatever tier each carries individually.** Phase 5 is the cleanest demonstration — its only P2 shipped with four P1s, and one of those P1s could not have shipped first at any priority. Phase 7 is the tidiest: a P1 skill and a P2 subagent, where splitting them would have shipped a documentation procedure with no handoff to feed and a handoff agent with no finished documentation to cite.
>
> **An empty queue changes what this section is for.** It stops being a plan and becomes a record — and the one thing a record has to keep doing is make the deferrals visible. `sweep-requirements-gaps` is the only entry left, it is not in the queue, and the reason it is not is written down in a form somebody can check without asking anyone.

> **Phase 2's seven are already built and are not in this queue** — `solution-architect`, `architecture-reviewer`, `accessibility-auditor`, `render-design-diagrams`, `data-model-design`, `api-contract-freeze` and `/adr` shipped as one phase set. ~~**The `threat-modeler` ↔ `architecture-reviewer` mutual exclusion is now one-sided and must be completed when `threat-modeler` ships.**~~ **Resolved on the Phase 5 build, and it needed no edit at all.** The shipped reviewer excludes threat modelling **by work type** rather than by slug — a genericisation made during the Phase 2 build to avoid a dangling pointer — so `threat-modeler` shipped with the reciprocal written the same way, in the same register, with zero cross-phase edits. The question this note left open (*"can both sides safely name each other?"*) answered itself: **neither needed to.** See [Phase 5 build — consequences](#phase-5-build--consequences) §1.
...
Build only if the frequency materialises. **One entry is left.** `sweep-requirements-gaps` carries a **falsifiable release trigger and a numeric drop condition** rather than a felt one (see its entry).

> **`handoff-agent` has left this queue, and its drop condition was overtaken rather than evaluated.** The condition was *"waits for the first real engagement close"* — a fact about **the calendar**, not about the artifact, and unlike `reproduce-and-diagnose-bug`'s per-repo condition it could not even be relocated to an install step, because a consuming repo cannot check it either. It shipped with the phase set for the ordinary dependency reason, and building it settled a question the condition never asked: whether the agent could honestly satisfy the gate it was aimed at. It cannot — **one of seven** — and that is knowable at build time, whereas *"has an engagement closed yet"* would have deferred the artifact indefinitely without ever surfacing it. **A drop condition indexed to an external event is not a test; it is a postponement with a date on it.** Compare `api-contract-freeze` and `/load-task-context`, whose conditions were questions building could answer.
...
| Quarterly KB Completeness Check | 281 | keep-as-prompt | `quarterly-kb-review` killed |
| KT Session Script | 321 | keep-as-prompt | narrowed out of `handoff-agent` |
| KT Session Summary | 369 | keep-as-prompt | — |
| ~~Inherited Error Triage~~ | ~~438~~ | **deleted** | Sentry removed — 9 of 12 CSV columns are error-tracker fields |
| Post-Handoff Retrospective | 436 | subagent | `handoff-agent` |

> **Four line numbers in this table were stale, and the deleted row is why.** Removing `Inherited Error Triage` shifted everything below it — `Post-Handoff Retrospective` by 59 lines — and the table was never re-derived. Re-verified against `PROMPTS.md` headings on the Phase 7 build. **A deletion recorded in a table is also an edit to every line number under it**, which is the one consequence a strikethrough row does not visibly carry.
