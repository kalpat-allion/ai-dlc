...
>
> **Scope: 40 shipped artifacts** — 7 development subagents, 2 DevOps subagents, 3 design subagents, 2 testing subagents, 3 security subagents, **1 delivery subagent**, 6 DevOps skills, 3 requirement skills, 3 design skills, 2 development skills, 2 testing skills, 2 security skills, **1 delivery skill**, and **3 commands** (1 design, 2 development). *(Counted from `aidlc-phases/*/{skill,subagent,command}-prompts/`, not incremented.)* **All seven phases have now shipped.** Proposed-but-unbuilt artifacts are listed in [PROMPT-CONVERSION-ANALYSIS.md](./PROMPT-CONVERSION-ANALYSIS.md) and enter this map when they ship, not before.
...
| Artifact | Type | Owns | Never owns | Provenance |
|---|---|---|---|---|
| `finalise-documentation` | Skill | The published documentation set as one loop: the section structure, every generated page, the spec-rendered API reference, the completeness audit and its gap list, the link check, and the stop before publish. Self-certifies **15 of 19** gate items, names the other 4 | One module's README or its inline comments, hand-written API reference content, the API specification itself, **any alert runbook**, the live knowledge base, the handoff document and the retrospective, **the publish itself** | P7 |
| `handoff-agent` | Subagent | One source-cited synthesis deliverable per invocation — the handoff document, or the post-handoff retrospective. Prints its artefact manifest, cites every claim inline, marks `[VERIFY: …]` and `[REDACT-BEFORE-SHARE]` rather than inventing or deleting. Self-certifies **1 of 7** gate items | Publishing anything, **any tracker write**, KB seeding and KT session scripts (both stay paste prompts), the documentation-site pages, an incident post-mortem, producing both its own deliverables in one run | P7 |

**These two split on write type and audience, and the seam is written that way on both sides rather than by slug.** The skill writes **pages on a published documentation site**; the agent writes **one synthesised deliverable for a receiving team** that is never published to that site. "Does it produce Markdown a human reads?" separates neither. The tests that do: *where it lands* and *who reads it*. **Neither names the other by slug on purpose** — this phase ships across two directories and installation is per-directory, so a same-phase slug dangles exactly as readily as a cross-phase one (the `author-test-plan` and `open-pull-request` rows below are the same lesson learned the expensive way).

**Both refuse the last step of their own loop, and that is the pattern to preserve.** `finalise-documentation` cannot publish what it wrote; `handoff-agent` cannot share what it synthesised or file its own next steps. Each would be more convenient merged with its terminal step, and each merge deletes the only human read in a phase whose entire risk profile is *confidently wrong content reaching a customer or a team that cannot check it*.
...
| "write the getting-started page" · "are our docs complete" · "what's missing from our docs site" | `finalise-documentation` | — | Free. Generation and the completeness audit are **one loop**, so the description is written as one procedure: entering at the audit end resumes there and loops back into generation for every Critical/High finding, and cannot terminate before the publish stop |
| "document this module" · "write the README for src/orders" | `/write-module-readme` — **explicitly fired only** | `finalise-documentation` | **A different and worse failure than an ordinary collision, and worth reading twice.** The command carries `disable-model-invocation: true`, so it would not *compete* for this utterance — it would silently **lose** it, and the developer mid-story would get a whole-documentation-site procedure. A collision at least has two claimants; this has one claimant that cannot answer. Closed in the skill's description **by work type, not by slug** — a cross-phase slug dangles in a repo that installed P7 without P3 |
| "we need docs before the handoff" | split on **write type** | — | Both artifacts' language in one sentence. Pages on a published site are `finalise-documentation`; one synthesised deliverable for the receiving team is `handoff-agent`. Both descriptions carry the qualifier; a bare *"docs"* with no site/deliverable qualifier is genuinely ambiguous and the operator should say which |
| "generate the handoff document" · "produce the post-handoff retrospective" | `handoff-agent` | — | Free among the 40. Name-invoked, so no auto-trigger competition |
| "write the post-mortem for last night's outage" | the repo's incident procedures | `handoff-agent` | **New adjacency, recorded on the P7 build.** Same noun family, different evidence and a different clock: a retrospective reports a **support period that has ended**; a post-mortem reports an **incident that just happened**. `secret-leak-response` and `reproduce-and-diagnose-bug` each own a post-mortem *seed* on their own trigger, and neither is reachable from *"write the retro"* — which is why the handoff agent excludes the post-mortem by name rather than relying on the noun to separate them |
| "write the runbook for the high-latency alert" | `observability-bringup` | `finalise-documentation` | **Lane reserved from the other side before the skill existed** — the observability skill owns alert runbooks by *file path*, so no coordinated edit was needed. The documentation skill links them and is forbidden from authoring or rewriting one: a documentation pass rewriting a runbook produces one that no longer matches the alert that fires it. **Deliberate, not an oversight — do not tidy either half out** |
| "hand-write the endpoint docs, the OpenAPI file is a pain" | *nothing* | `finalise-documentation` | Refused. The specification is the source and the reference renders from it; the skill also refuses the reverse — editing the spec to make a page easier to write. Both directions invert the flow `api-contract-freeze` established one phase over |
| "write us a DR plan" | *nothing* | `finalise-documentation` | Refused, and reported as a gap with a named owner. Recovery objectives are stated by people, not derivable from a repository — a generated DR plan reads authoritative and has never been tested. **Same finding that killed `/infra-runbook`, reached from a second direction** |
| "just put a coverage number in, it's around 80%" | *nothing* | `handoff-agent` | `[VERIFY: …]`, never an estimate. The figure is read from the coverage artefact or it is not stated — and a handoff document's numbers are planned against for months by people who cannot re-derive them |
...
| write the getting-started page, are our docs complete, what's missing from our docs site | `finalise-documentation` — **every trigger names the documentation set or the docs site.** Never shorten one to a bare "document this" or "write the docs for X"; that is the module-documentation command's territory and it cannot win the utterance back |
| write the runbook *(alert-triggered)* | `observability-bringup`. **Not `finalise-documentation`**, which links alert runbooks and is forbidden from authoring one |
| generate the handoff document, synthesise the handoff, produce the post-handoff retrospective | `handoff-agent` — **every trigger names the handoff or the retrospective.** Never a bare "write the docs for the handover" |
| write the post-mortem | **nobody, as a general verb.** Two artifacts own a post-mortem *seed* on their own narrow triggers (a credential incident; a triaged bug), and `handoff-agent` excludes the post-mortem by name. A bare "write the post-mortem" is an incident-process act with a named human owner |
| publish the docs, deploy the docs site, share the handoff doc with the recipient | **nobody** — both P7 artifacts stop one step short by design, on the same footing as the apply credential and the risk-acceptance signature |
...
| `finalise-documentation` | a project-scope `documentation`, `docs-writer` or `technical-writer` skill | **Predicted, not observed, and an install-time decision rather than a build-time fix.** Both would claim *"write the getting-started page"*, and skills do not compete — the router picks one silently. **The direction that hurts is the incumbent winning**: the pages still appear, so the surface looks identical, while the completeness loop, the spec-rendered API reference and the stop-before-publish never run. Nothing downstream reports their absence. Mitigated only in that every shipped trigger names the documentation *set* or the docs *site* rather than a bare "write the docs". Run the negative-routing check after install and decide which one the repo keeps |
| `finalise-documentation` | `/write-module-readme` **as shipped today** | **Recorded as a non-collision that is one frontmatter field away from being one.** The command carries `disable-model-invocation: true`, so it has no auto-trigger surface and the skill's by-work-type exclusion is belt to that brace. Remove the field and *"document this module"* becomes a live two-claimant utterance — with the skill winning, because skills beat commands on auto-trigger. **Verify the field is present rather than assuming the directory grants it** |
| `handoff-agent` / `finalise-documentation` | **a partial Phase 7 install** | Same shape as the `author-test-plan` and `threat-model-verifier` rows, and **pre-empted at build**: neither template names the other by slug, precisely because installation is per-directory. The residual is not a dangling pointer but a **silent scope gap** — a repo installing `subagent-prompts/` alone gets handoff synthesis with no documentation loop in front of it, and the handoff document then cites a docs site whose completeness nobody checked. **Copy both directories, or know which half you are running without** |
