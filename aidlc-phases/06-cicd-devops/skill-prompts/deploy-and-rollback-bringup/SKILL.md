...
These are the least-verified controls a repository has, because almost all of them are only exercised during a real production deploy and the one that is not — the drill — is the one everybody defers. So the drill happens now, not next quarter.
...
2. Confirm staging auto-deploys on merge to the default branch — same workflow, OIDC cloud authentication, no human in the loop. **The delivery-pipeline bring-up owns that workflow**; wire it here only if it is absent, and never replace a working one. You need it to exist because step 8's drill runs through it.
...
8. **Run the drill against a staging release candidate now, not next quarter.** Measure wall-clock time to healthy, record it, and schedule the quarterly repeat. → **the deployment gate: deployment and rollback.**
