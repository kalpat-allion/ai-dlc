...
2. Draft a step-by-step refactor plan (one mechanical primitive per step — extract / rename / inline / parameterise / upgrade) and surface it before editing. The developer approves the plan; only then do you proceed.
...
4. If the new structure introduces a seam without test coverage, add tests for it — these are characterisation tests, not new behaviour.
...
- The refactor target spans a service boundary, a public API contract, or a datastore schema → escalate to human architect review before proceeding.
- Pre-existing tests fail before any edit → stop; the branch is broken and refactoring on a broken baseline is unsafe.
- The developer asks you to bundle with feature work → refuse and explain the strict separation rule (one refactor PR maps to one `tech-debt` issue, never bundled with feature work).
